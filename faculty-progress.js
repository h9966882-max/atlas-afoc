(() => {
  ...
  style.textContent = `
    ...
    .faculty-top{display:grid;grid-template-columns:1.35fr repeat(3,minmax(110px,.55fr));gap:8px;margin-bottom:10px}
    .faculty-identity,.faculty-stat{border:1px solid #eaded6;border-radius:16px;background:#fff;padding:11px 12px}
    .faculty-identity{background:linear-gradient(135deg,#f4ece6,#fbf8f4)}.faculty-identity .label{font-size:8px;color:#9a837b;font-weight:900}.faculty-identity b{display:block;font-size:18px;margin:2px 0}.faculty-identity span{font-size:9px;color:#7a716b}
    .faculty-stat small{display:block;font-size:7px;color:#9b8e87;font-weight:900;margin-bottom:3px}.faculty-stat strong{display:block;font-size:16px;letter-spacing:-.03em}.faculty-stat em{display:block;font-style:normal;font-size:7.5px;color:#7d746f;margin-top:2px}
    ...
    .curriculum-alert{margin-top:8px;border:1px solid #ead5a6;background:#fff8df;border-radius:12px;padding:8px 9px;font-size:8px;line-height:1.55;color:#6e6044}.curriculum-alert b{font-size:8.5px}
    ...
    @media(max-width:760px){.faculty-top{grid-template-columns:1fr 1fr}.faculty-identity{grid-column:1/-1}.material-lanes{grid-template-columns:1fr}.phase-strip{grid-template-columns:repeat(4,minmax(0,1fr))}.curriculum-board{padding:8px 7px 1px}.curriculum-head{padding:12px}.curriculum-content{padding:10px}.curriculum-head h2{font-size:16px}}
    @media(max-width:430px){.faculty-top{grid-template-columns:1fr}.faculty-identity{grid-column:auto}.phase-strip{grid-template-columns:repeat(2,minmax(0,1fr))}}
  `;
  ...
  root.innerHTML = '<div class="curriculum-shell"><div class="curriculum-loading">📚 学部カリキュラム正本を読み込み中…</div></div>';
  ...
  let snapshot = { faculties: [], rooms: [], health: [] };
  let channel = null;
  ...
  function renderFaculty(f){
    ...
    const canonDate=f.canon_checked_at?new Date(f.canon_checked_at).toLocaleString('ja-JP',{timeZone:'Asia/Tokyo',month:'numeric',day:'numeric',hour:'2-digit',minute:'2-digit'}):'未記録';
    ...
    return `<article class="faculty-progress-card">
      ...
      <div class="curriculum-foot">正本集計：${esc(canonDate)} JST｜固定総数・正式状態＝Notion正本、現在作業＝開発室Live。${f.note?` ${esc(f.note)}`:''}</div>
    </article>`;
  }
  ...
  function render(){
    ...
    root.innerHTML=`<div class="curriculum-shell">
      <header class="curriculum-head"><div><div class="curriculum-eyebrow">ATLAS CURRICULUM DEVELOPMENT MAP</div><h2>📚 学部カリキュラム開発進捗</h2><p>「全部でどれだけある？ 今どこ？ 何をしてる？ あとどれだけ？」を正本＋Liveで見る。</p></div><button class="curriculum-toggle" type="button">${collapsed?'📚 進捗を開く':'▴ たたむ'}</button></header>
      <div class="curriculum-content" ${collapsed?'hidden':''}>${faculties.length?faculties.map(renderFaculty).join(''):'<div class="curriculum-loading">正本集計済みの学部はまだないよ。</div>'}</div>
    </div>`;
  ...
  async function load(){
    ...
    const [fResult,rResult,hResult]=await Promise.all([
    ...
    if(fResult.error||rResult.error||hResult.error){
      console.error('faculty progress load failed',fResult.error||rResult.error||hResult.error);return;
    }
    snapshot={faculties:fResult.data||[],rooms:rResult.data||[],health:hResult.data||[]};
    render();
  }

  async function startRealtime(){
    if(channel)await client.removeChannel(channel);
    channel=client.channel('afoc-faculty-curriculum')
